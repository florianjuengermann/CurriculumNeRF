"""
SOURCE: https://github.com/ActiveVisionLab/nerfmm
"""
...
def volume_sampling(c2w, ray_dir_cam, t_vals, near, far, perturb_t):
    ...
    if perturb_t:
        ...
        t_noise = torch.rand(
            (ray_H, ray_W, N_sam), device=c2w.device, dtype=torch.float32)  # (H, W, N_sam)
    ...
    sample_pos = ray_ori_world.view(
        1, 1, 1, 3) + ray_dir_world.unsqueeze(2) * t_vals_noisy.unsqueeze(3)

    # (H, W, N_sample, 3), (3, ), (H, W, 3), (H, W, N_sam)
    return sample_pos, ray_ori_world, ray_dir_world, t_vals_noisy
...
def volume_sampling_ndc(c2w, ray_dir_cam, t_vals, near, far, H, W, focal, perturb_t):
    ...
    ray_ori_world = ray_ori_world.view(1, 3).expand_as(
        ray_dir_world)  # (3, ) -> (1, 3) -> (H*W, 3)
    if isinstance(focal, float):
        ray_ori_world, ray_dir_world = get_ndc_rays(
            H, W, focal, 1.0, rays_o=ray_ori_world, rays_d=ray_dir_world)  # (H*W, 3)
    ...
    sample_pos = ray_ori_world.unsqueeze(
        2) + ray_dir_world.unsqueeze(2) * t_vals_noisy.unsqueeze(3)
...
def volume_rendering(rgb_density, t_vals, sigma_noise_std, rgb_act_fn):
    ...
    dist_far = torch.empty(size=(ray_H, ray_W, 1), dtype=torch.float32,
                           device=dists.device).fill_(1e10)  # (H, W, 1)
    ...
    acc_transmittance = torch.cumprod(
        1.0 - alpha + 1e-10, dim=2)  # (H, W, N_sample)
    acc_transmittance = torch.roll(
        acc_transmittance, shifts=1, dims=2)  # (H, W, N_sample)
